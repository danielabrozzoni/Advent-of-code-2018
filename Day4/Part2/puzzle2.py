...
def defineSleepyMinute(shifts):
	...
	for shift in shifts:
		if re.match(r'Guard (.*) begins shift', shift.description):
			...
			while currentDate < endDate:
				...
				if currentGuard not in minutes:
					minutes[currentGuard] = {}
				if timeStamp in minutes[currentGuard]:
					minutes[currentGuard][timeStamp] += 1
				else: 
					minutes[currentGuard][timeStamp] = 1
				currentDate += delta
	...
	for guard in minutes:
		#print guard
		#print minutes[guard]
		for time in minutes[guard]:
			if mostFrequentTimes < minutes[guard][time]:
				mostFrequentTimes = minutes[guard][time]
				mostFrequentMinute = int(time[2:])
				mostFrequentGuard = guard

	return mostFrequentGuard * mostFrequentMinute

def main():
	shifts = createShifts()	
	sleepyMinute = defineSleepyMinute(shifts)
	print sleepyMinute
main()